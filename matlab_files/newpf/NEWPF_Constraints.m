function [c, ceq] = NEWPF_Constraints(x,A2t,A3t,Delta,eff_K,eff_L,K0,N,R0,S1_2000,S2_2000,Sbar,T,alpha,beta,gZ_en,gZd_y,gZBGP,gamma,kappa1,kappa2,kappa3,phi,phi0,phiL,rho,sigma,ypsilon)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%      Section 4: Constraints                                 %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%function [c, ceq] = NEWPF_Constraints(x,A2t,A3t,At,Delta,K0,N,R0,S1_2000,S2_2000,Sbar,T,alpha,beta,gZ_en,gZd_y,gZBGP,gamma,kappa1,kappa2,kappa3,phi,phi0,phiL,rho,sigma,v,ypsilon)
...
for i = 1:1:T;
    c(i) = (-1)*(Ct(i)-0.0001); % These use indices 1 to T
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Labor Share Sum Constraint: %%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % Constraint: labor_share_coal (x(2*(T-1)+i)) + labor_share_wind (x(2*(T-1)+T+i)) <= 1
%for i = 1:1:T
    %c(2*T+1+i) = x(2*(T-1)+i) + x(2*(T-1)+T+i) - 1;
%end
